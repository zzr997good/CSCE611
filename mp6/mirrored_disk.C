...
void MirroredDisk::read(unsigned long _block_no, unsigned char * _buf) {
  ...
  while(!master->is_ready()&&!dependent->is_ready()){             //Both two disks are not ready for reading
...
void MirroredDisk::write(unsigned long _block_no, unsigned char * _buf) {
  ...
  while(!master->is_ready()&&!dependent->is_ready()){             //Both two disks are not ready for reading
        SYSTEM_SCHEDULER->resume(Thread::CurrentThread());              //Add the thread to teh ready queue again
        Console::puts("Two disks are not ready, voluntarily yielding thread\n ");  
        SYSTEM_SCHEDULER->yield();
  }
  if(master->is_ready()){
      ...
      while(!dependent->is_ready()){             //Both two disks are not ready for reading
        SYSTEM_SCHEDULER->resume(Thread::CurrentThread());              //Add the thread to teh ready queue again
      ...
      //Dependent disk is ready, write to it , and wait for dependent disk
      ...
      while(!master->is_ready()){             //Both two disks are not ready for reading
        SYSTEM_SCHEDULER->resume(Thread::CurrentThread());              //Add the thread to teh ready queue again
        Console::puts("Dependent disk is not ready, voluntarily yielding thread\n ");  
        SYSTEM_SCHEDULER->yield();
      }
